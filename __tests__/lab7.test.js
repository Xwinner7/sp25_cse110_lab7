describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const button = await page.$$eval('product-item', prodItems => 
    prodItems.map(item => item.shadowRoot.querySelector('button').innerText));

    const allButtonCorrect = button.every(text => text === 'Remove from Cart');
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for(const item of prodItems) {
      ...
      if(buttonText === 'Remove from Cart') {
        await button.click();
        await page.waitForFunction(
          (button) => button.innerText === 'Add to Cart',
          {},
          button
        );
      }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allButtonCorrect = button.every(text => text === 'Add to Cart');
    expect(allButtonCorrect).toBe(true);
  }, 15000);
